...
// 配置参数（可通过命令行覆盖）
static int target_conn = 10000;      // 目标连接数
static int port = 8888;              // 服务器端口
static const char *server_ip = "127.0.0.1";
static int test_mode = 0;            // 0: 仅连接测试, 1: 吞吐测试
static int req_per_conn = 0;         // 每个连接发送请求次数（0表示无限循环）
static int batch_size = 100;         // 每次 epoll_wait 最大事件数

static volatile int running = 1;

// 统计信息
static struct {
    long long conn_attempts;     // 尝试连接次数
    long long conn_success;      // 成功连接数
    long long conn_fail;         // 失败连接数
    long long req_sent;          // 发送请求次数
    long long req_recv;          // 收到响应次数
    long long total_latency_us;  // 总延迟（微秒）
...
struct Conn {
    ...
    int pending_req;   // 0: 空闲, 1: 已发送等待响应
    int req_count;     // 已发送请求数（用于限制）
};
...
// 添加 fd 到 epoll
static void add_epoll(int fd, uint32_t events) {
    struct epoll_event ev;
    ev.events = events;
    ev.data.fd = fd;
...
// 修改 fd 的事件
static void mod_epoll(int fd, uint32_t events) {
    struct epoll_event ev;
    ev.events = events;
    ev.data.fd = fd;
...
static void send_request(Conn *c) {
    // 构造简单请求（可自定义）
    snprintf(c->send_buf, sizeof(c->send_buf), "ping %d\n", c->fd);
    c->send_len = strlen(c->send_buf);
    int n = send(c->fd, c->send_buf, c->send_len, 0);
    if (n == c->send_len) {
        c->pending_req = 1;
        gettimeofday(&c->send_time, NULL);
        __sync_fetch_and_add(&stats.req_sent, 1);
        // 等待响应，暂时不关注写事件
        mod_epoll(c->fd, EPOLLIN);
    } else {
        // 发送失败，关闭连接
        close(c->fd);
...
static void handle_recv(Conn *c) {
    ...
    buf[n] = '\0';
    __sync_fetch_and_add(&stats.req_recv, 1);
    if (c->pending_req) {
        struct timeval now;
        gettimeofday(&now, NULL);
        long long us = (now.tv_sec - c->send_time.tv_sec) * 1000000 +
                       (now.tv_usec - c->send_time.tv_usec);
        __sync_fetch_and_add(&stats.total_latency_us, us);
        c->pending_req = 0;
        // 如果还需要继续发送，则重新关注写事件
        if (test_mode == 1 && (req_per_conn == 0 || c->req_count < req_per_conn)) {
            mod_epoll(c->fd, EPOLLOUT);
            c->req_count++;
        } else {
            // 不需要再发送，只关注读（保持连接）
            mod_epoll(c->fd, EPOLLIN);
...
static void try_connect() {
    ...
    __sync_fetch_and_add(&stats.conn_attempts, 1);
    if (ret == 0) {
        // 立即连接成功（极少）
        __sync_fetch_and_add(&stats.conn_success, 1);
        Conn *c = &conns[sockfd];
        c->fd = sockfd;
        c->send_len = 0;
        c->recv_len = 0;
        c->pending_req = 0;
        c->req_count = 0;
        add_epoll(sockfd, test_mode == 1 ? EPOLLOUT : EPOLLIN);
        if (test_mode == 1) {
            // 马上发送第一个请求
            send_request(c);
        }
        ...
        c->pending_req = 0;
        c->req_count = 0;
        add_epoll(sockfd, EPOLLOUT);
    } else {
        // 连接失败
        __sync_fetch_and_add(&stats.conn_fail, 1);
        close(sockfd);
    }
}

// 处理连接完成（写事件）
static void handle_connect(Conn *c) {
    int err;
...
// 打印统计信息（每隔一秒）
static void* stats_printer(void*) {
    struct timeval last = {0,0};
    long long last_conn = 0, last_req = 0;
    while (running) {
        sleep(1);
        struct timeval now;
        gettimeofday(&now, NULL);
        long long conn_success = __sync_fetch_and_add(&stats.conn_success, 0);
        long long req_recv = __sync_fetch_and_add(&stats.req_recv, 0);
        if (last.tv_sec == 0) {
            last = now;
            last_conn = conn_success;
            last_req = req_recv;
            continue;
        }
        double elapsed = (now.tv_sec - last.tv_sec) + (now.tv_usec - last.tv_usec) / 1000000.0;
        double conn_rate = (conn_success - last_conn) / elapsed;
        double qps = (req_recv - last_req) / elapsed;
        fprintf(stderr, "[%ld] connections: %lld (+%.1f/s), requests: %lld (%.1f/s)\n",
                (long)time(NULL), conn_success, conn_rate, req_recv, qps);
        last = now;
        last_conn = conn_success;
        last_req = req_recv;
    }
...
int main(int argc, char **argv) {
    // 解析命令行参数
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <server_ip> <port> [mode] [target_conn] [req_per_conn]\n", argv[0]);
        fprintf(stderr, "  mode: 0 = only connection test, 1 = throughput test (default 0)\n");
        fprintf(stderr, "  target_conn: target number of connections (default 10000)\n");
        fprintf(stderr, "  req_per_conn: requests per connection (0 = infinite, default 0)\n");
        return 1;
    }
    server_ip = argv[1];
    port = atoi(argv[2]);
    if (argc > 3) test_mode = atoi(argv[3]);
    if (argc > 4) target_conn = atoi(argv[4]);
    if (argc > 5) req_per_conn = atoi(argv[5]);
    ...
    // 分配连接数组（按最大 fd 值，简单起见用数组）
    int max_fd = target_conn * 2 + 100;  // 预估最大 fd
    conns = (Conn*)calloc(max_fd, sizeof(Conn));
    if (!conns) {
        perror("calloc");
        return 1;
    }
    ...
    // 启动统计线程
    pthread_t printer;
    pthread_create(&printer, NULL, stats_printer, NULL);

    struct timeval tv_begin;
    gettimeofday(&tv_begin, NULL);
    stats.start_time = tv_begin;

    struct epoll_event events[4096];
    // 修正：使用实际成功连接数作为循环条件
    while (running && stats.conn_success < target_conn) {
        // 批量发起连接
        int to_create = target_conn - stats.conn_success;
        if (to_create > batch_size) to_create = batch_size;
        for (int i = 0; i < to_create; i++) {
            try_connect();
        }
        // 处理事件
        int nfds = epoll_wait(epfd, events, 4096, 10);
        for (int i = 0; i < nfds; i++) {
    ...
    double total_sec = (tv_end.tv_sec - tv_begin.tv_sec) + (tv_end.tv_usec - tv_begin.tv_usec) / 1000000.0;
    printf("\n=== Final Statistics ===\n");
    ...
    if (test_mode == 1) {
        printf("Requests sent: %lld\n", stats.req_sent);
        printf("Responses received: %lld\n", stats.req_recv);
        if (stats.req_recv > 0) {
            printf("Average latency: %.3f ms\n", (double)stats.total_latency_us / stats.req_recv / 1000.0);
            printf("QPS: %.2f\n", (double)stats.req_recv / total_sec);
        }
    }
