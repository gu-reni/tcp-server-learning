根据你提供的最终测试结果（8 线程版本），我已将 README 中的压测数据、技术栈说明和系统调优建议更新为最新版本，并补充了关键性能指标。修改后的 README 如下：

---

# Linux C++ 高并发 TCP 服务器学习笔记

本项目是我从零开始学习 Linux 下 C++ 高并发网络编程的完整记录。通过逐步实现单线程阻塞、多线程、I/O 多路复用（epoll 水平触发/边缘触发）、非阻塞 I/O 以及多线程 epoll 模型，深入理解 TCP 服务器的高并发实现原理。最终版本实现了支持 `sendfile` 零拷贝的静态文件服务器，并在虚拟机环境下完成了系统调优与压力测试，在 **2 核 4GB** 虚拟机中达到 **6.4 万并发长连接** 和 **9000+ QPS** 的吞吐能力。

## 📚 技术栈

- **语言**：C++17
- **网络模型**：epoll（边缘触发） + 多线程 Reactor（8 个工作线程）
- **I/O 优化**：非阻塞 I/O、sendfile 零拷贝
- **协议**：HTTP/1.1（静态文件服务）
- **构建**：g++、CMake
- **压测**：wrk、ab（Apache Bench）、自研高并发客户端（`stress_client`）
...
## 📊 压测结果（虚拟机 2核4GB，8 个工作线程）

| 压测工具 | 场景 | 并发数 | QPS | 平均延迟 | 50% 延迟 | 99% 延迟 | 备注 |
|----------|------|--------|-----|----------|----------|----------|------|
| **stress_client** | 仅连接 | 目标 64500 | — | — | — | — | 成功 **63976** 连接，达单 IP 端口上限 |
| **stress_client** | 吞吐（长连接） | 30098 连接 | 1422 | 16.7 ms | — | — | 每连接循环请求 |
| **ab** | 短连接 | 1000 | **9120** | 109.6 ms | 52 ms | 751 ms | QPS 显著提升 |
| **ab** | 短连接 | 100 | 5678 | 17.6 ms | 20 ms | 97 ms | 低并发下延迟极低 |
| **wrk** | 长连接 | 1000 | **9279** | 143.7 ms | 61.9 ms | 719.8 ms | 多线程长连接，性能稳定 |
| **wrk** | 长连接 | 2000 | **9511** | 274.8 ms | 129.0 ms | 1.32 s | 并发翻倍，QPS 未降，延迟可控 |

> 详细压测记录见 [Project Diary/项目经过.md](Project%20Diary/项目经过.md) 第四、六、七阶段。
...
| 高并发下 QPS 瓶颈 | 将工作线程从 4 增至 8，减少锁竞争，QPS 提升约 8%~25% |
