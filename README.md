...
6. **静态文件服务器（sendfile 零拷贝）**  
   - 基于多线程 epoll 模型，解析 HTTP 请求并返回静态文件  
   - 使用 `sendfile` 实现零拷贝文件传输，避免用户空间拷贝  
   - 正确处理非阻塞发送（EAGAIN），支持大文件传输
...
- 06-static-file-server-sendfile/
  - tcp_server.cpp          # 静态文件服务器（sendfile 零拷贝）
  - index.html              # 示例静态文件
- Project_Diary.md
- README.md
## 编译与运行
...
- 系统参数调整对高并发的重要性  
- 零拷贝（`sendfile`）的实现原理及其性能优势  
- 非阻塞发送中状态机（响应头 + 文件体）的设计与实现

## 下一步计划

- 解析 HTTP 请求路径，支持多个静态文件（如 CSS、JS、图片）  
- 使用内存映射（`mmap`）结合 `sendfile` 进一步优化  
- 使用线程池分离业务逻辑（如文件 I/O 与 HTTP 解析）  
